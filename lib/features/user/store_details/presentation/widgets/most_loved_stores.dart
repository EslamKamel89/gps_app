...
import 'package:gps_app/features/user/store_details/cubits/stores_cubit.dart';
import 'package:gps_app/features/user/store_details/presentation/store_details_screen.dart';

class MostLovedStoresProvider extends StatelessWidget {
  const MostLovedStoresProvider({super.key, required this.isStore});
  final bool isStore;
  @override
  Widget build(BuildContext context) {
    return BlocProvider(
      create: (context) => StoresCubit()..stores(isStore: isStore),
      child: MostLovedStoresWidget(isStore: isStore),
    );
  }
}

class MostLovedStoresWidget extends StatefulWidget {
  const MostLovedStoresWidget({super.key, required this.isStore});
  final bool isStore;
  @override
  State<MostLovedStoresWidget> createState() => _MostLovedStoresWidgetState();
}

class _MostLovedStoresWidgetState extends State<MostLovedStoresWidget> {
  @override
  Widget build(BuildContext context) {
    return BlocBuilder<StoresCubit, ApiResponseModel<List<UserModel>>>(
          ...
          child: Column(
            ...
            children: [
              Padding(
                ...
                child: Text(
                  "Most ❤️ ${widget.isStore ? "Stores" : 'Farms'}",
...
class _StoreItem extends StatelessWidget {
  const _StoreItem({required this.user, required this.onTap});
  ...
  @override
  Widget build(BuildContext context) {
    final imageUrl = getImageUrl(user.image?.path);
    return InkWell(
