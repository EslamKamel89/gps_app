...
class AboutController {
  ...
  Future<ApiResponseModel<AboutModel>> getAbout() async {
    final t = prt('getAbout - AboutController');
    try {
      final response = await _api.get(EndPoint.about);
      pr(response, '$t - response');
      final List<AboutModel> models =
          (response as List).map((json) => AboutModel.fromJson(json)).toList();
      return pr(
        ApiResponseModel(response: ResponseEnum.success, data: models[0]),
        t,
      );
    } catch (e) {
